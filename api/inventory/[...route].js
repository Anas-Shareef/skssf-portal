...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Checkouts Routes
    if (route[0] === 'checkouts') {
      if (method === 'GET') {
        // Members see their own only
        if (route[1] === 'mine') {
          const { data, error } = await supabase
            .from('inventory_checkouts')
      ...
      if (method === 'POST') {
        ...
        // Stock race condition check: conditional update
      ...
      const id = route[1];
      if (!id) return res.status(400).json({ error: 'Checkout ID required' });

      // Check-in / Return routes
      if (method === 'PATCH') {
        if (route[2] === 'checkin') {
          ...
          // Increment available stock ONLY for 'good' condition returns
        ...
        // Admin manual override return
        if (route[2] === 'mark-returned') {
          if (profile.role !== 'admin' && profile.role !== 'super') {
            return res.status(403).json({ error: 'Forbidden' });
          }
